...
export async function auditVisiblePhotoDuplicates({
  ...
  const duplicates = [];
  for (const group of groupBy(placements, (placement) => placement.resolvedPath).values()) {
  ...
  for (const [hash, group] of groupBy(hashedPlacements, (placement) => placement.hash)) {
